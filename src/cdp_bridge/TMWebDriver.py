...
class TMWebDriver:  
    ...
    def start_http_server(self):
        ...
        def long_poll():
            ...
            if session_id not in self.sessions: 
                ...
                log(f"Browser http connected: {session.url} (Session: {session_id})")  
        ...
        def link():
            ...
            if data.get('cmd') == 'execute_js':
                ...
                try:
                    ...
                    log('[remote result]', (str(code)[:50] + ' RESULT:' +str(result)[:50]).replace('\n', ' '))
            ...
            def handle(self) -> None:  
                try:  
                    ...
                    if data.get('type') in ['ext_ready', 'tabs_update']:
                        ...
                        log(f"Received tabs update: {current_tab_ids}")
        ...
        if not session or not session.is_active(): 
            ...
            if not session or not session.is_active(): 
                ...
                if alive_sessions:
                    ...
                    log(f"会话 {session_id} 未连接，自动切换到最新活动会话: {session.id}")
    ...
    def set_session(self, url_pattern: str) -> bool:  
        ...
        if not matched: return log(f"警告: 未找到URL包含 '{url_pattern}' 的会话")  
        if len(matched) > 1: log(f"警告: 找到多个URL包含 '{url_pattern}' 的会话，选择第一个")  
        self.default_session_id, info = matched[0]
        log(f"成功设置默认会话: {self.default_session_id}: {info['url']}")  
